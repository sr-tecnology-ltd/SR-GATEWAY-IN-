...
async function startServer() {
  ...
  const getDb = () => {
    ...
    try {
      ...
      if (!apps.length) {
        // If production env variables are provided
        if (process.env.FIREBASE_PROJECT_ID && process.env.FIREBASE_CLIENT_EMAIL && process.env.FIREBASE_PRIVATE_KEY) {
          ...
          if (fs.existsSync(configPath)) {
            ...
            adminObj.initializeApp({ projectId: config.projectId });
            if (config.firestoreDatabaseId) {
              console.log(`Setting up custom database instance: ${config.firestoreDatabaseId}`);
              dbInstance = getFirestore(config.firestoreDatabaseId);
            } else {
              dbInstance = getFirestore();
            }
          } else {
        ...
        // If app already initialized, look check for config to instantiate correct db
        const configPath = path.join(process.cwd(), "firebase-applet-config.json");
        if (fs.existsSync(configPath)) {
          const config = JSON.parse(fs.readFileSync(configPath, "utf-8"));
          if (config.firestoreDatabaseId) {
  ...
  app.all("/api/pay", async (req, res) => {
    ...
    try {
      ...
      await sendTelegramAlert(
        ...
        `💰 Disbursed: ₹${numAmount}\n` +
        `📞 Receiver Contact: <code>${cleanMobile}</code> (${receiverData.displayName})\n` +
        `🧾 Transaction ID: <code>${generatedTxnId}</code>\n` +
        `🟢 Status: SUCCESS (Automated processing)`
      );
  ...
  app.all("/api/system/transfer", async (req, res) => {
    ...
    try {
      ...
      await db.runTransaction(async (transaction) => {
        ...
        // Save transaction record to receiver
        const receiverTxnRef = db.collection("transactions").doc();
        transaction.set(receiverTxnRef, {
          userId: userDoc.id,
          mobile: cleanMobile,
          id: txnId,
          type: mode === "wallet" ? "transfer-received" : "deposit",
          amount: amountVal,
          ...
          description: mode === "wallet" 
            ? `Received from Admin ${adminData.displayName}` 
            : `System credit deposited by Admin (${adminData.displayName || "System Admin"})`
        });

        // Save transaction record to admin/system sender
        ...
        transaction.set(adminTxnRef, {
          ...
          timestamp: FieldValue.serverTimestamp(),
          description: mode === "wallet" 
            ? `Admin wallet disbursed code to ${userData.displayName}` 
            : `System balance credit dispatch to ${userData.displayName} (via API)`
        });
      ...
      // Dispense Telegram alert
      await sendTelegramAlert(
        `<b>⚡ Automated Balance Credit Dispatch! (@SRGatewayBot)</b>\n\n` +
        `👤 Administrator: ${adminData.displayName}\n` +
        `📱 Target Mobile: <code>${cleanMobile}</code>\n` +
        `👤 Target Name: ${userData.displayName}\n` +
        `💰 Disbursed Amount: <b>₹${amountVal}</b>\n` +
        `💎 Distribution Mode: <b>${mode.toUpperCase()}</b>\n` +
        `🧾 Transaction ID: <code>${txnId}</code>\n` +
